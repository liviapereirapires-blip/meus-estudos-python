...
TIPOS_F1 = [
    ...
    ("nimp", lambda p, q: not ((not p) or q),   lambda p, q: f"¬({_l(p)} → {_l(q)})"),
    ("nbic", lambda p, q: not (p == q),         lambda p, q: f"¬({_l(p)} ↔ {_l(q)})"),
]
...
FORMULAS = [
    # ── Tautologias (sempre usam P e Q) ──────────────
    ("(P∧Q) → P",      lambda p, q, r: not (p and q) or p,           "Tautologia", "imp"),
    ("(P∧Q) → Q",      lambda p, q, r: not (p and q) or q,           "Tautologia", "imp"),
    ("P → (Q → P)",    lambda p, q, r: not p or (not q or p),        "Tautologia", "imp"),
    ("¬P → (P → Q)",   lambda p, q, r: p or (not p or q),            "Tautologia", "imp"),
    ("(P→Q) ∨ (Q→P)",  lambda p, q, r: (not p or q) or (not q or p), "Tautologia", "imp"),
    ("(P∧¬P) → Q",     lambda p, q, r: not (p and not p) or q,       "Tautologia", "neg"),

    # ── Contradições (sempre usam P e Q) ─────────────
    ("(P→Q) ∧ P ∧ ¬Q", lambda p, q, r: (not p or q) and p and not q,          "Contradição", "imp"),
    ("(P↔Q) ∧ P ∧ ¬Q", lambda p, q, r: (p == q) and p and not q,              "Contradição", "bic"),
    ("P ∧ ¬P ∧ Q",      lambda p, q, r: p and not p and q,                     "Contradição", "neg"),
    ("(P∧Q) ∧ ¬(P∨Q)",  lambda p, q, r: (p and q) and not (p or q),            "Contradição", "and"),
